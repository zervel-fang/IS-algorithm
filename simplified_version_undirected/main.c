...
int main(int agrs, char **argv){
	...
	srand(time(NULL)) ;
	Get_size_of_vertex_and_edges(argv) ;
	Init()  ; 
	Input_Network(argv) ;
	Input_Fstar_and_Gstar(argv) ;
	Init_Influence_and_Susceptibility() ;
	time_t t1 ,t2;
	t1 = time(NULL)	 ;
	Iterate_Influence_label(atoi(argv[3]),argv) ;
	t2 = time(NULL) ;
	Generate_pij_weight() ;
	Output(argv) ;
	Output_detaildata(argv) ;
	return 0 ;
}
void Get_size_of_vertex_and_edges(char **argv)
{
...
void Input_Fstar_and_Gstar(char **argv) 
{
	...
	char f2[5] = ".txt" ;
	file_name1 = (char *) malloc((strlen(argv[1])+strlen(f1)+strlen(f2)+2)*sizeof(char)) ;
	file_name2 = (char *) malloc((strlen(argv[1])+strlen(f3)+strlen(f2)+2)*sizeof(char)) ;
	sprintf(file_name1,"%s%s%s",f1,argv[1],f2) ;
	sprintf(file_name2,"%s%s%s",f3,argv[1],f2) ;
	printf("f1 is:%s\n",file_name1) ;
	FILE *F1, *F2 ;
...
void Iterate_Influence_label(int Iteration, char **argv)
{
	...
	{  
		...
		fprintf(fout,"%d %f %f\n",jj,value1,value2) ;
		if(value1<0.0001&&value2<0.0001)
		{	
			break ;
		}
		else
